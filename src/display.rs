...
pub fn top_files(conn: &Connection, limit: usize, min_size: u64) -> Result<()> {
    ...
    let rows = stmt.query_map(
        duckdb::params![min_size as i64, limit as i64],
    ...
    for row in rows.flatten() {
        println!("{:<80} {:>12}", truncate(&row.0, 80), format_size(row.1 as u64, BINARY));
    }
    Ok(())
}
...
pub fn by_extension(conn: &Connection, limit: usize) -> Result<()> {
    ...
    println!("{}", "-".repeat(46));
    for row in rows.flatten() {
        println!("{:<20} {:>10} {:>14}", row.0, row.1, format_size(row.2 as u64, BINARY));
    }
    Ok(())
}
...
pub fn find_files(conn: &Connection, pattern: &str, limit: usize) -> Result<()> {
    ...
    let rows = stmt.query_map(
        duckdb::params![sql_pattern, limit as i64],
        |row| Ok((row.get::<_, String>(0)?, row.get::<_, i64>(1)?)),
    )?;
...
pub fn export_html_report(conn: &Connection, db_path: &str) -> Result<()> {
    ...
    let mut stmt = conn.prepare(
        "SELECT path, size FROM files WHERE is_dir=false ORDER BY size DESC LIMIT 20"
    )?;
    for row in stmt.query_map([], |r| Ok((r.get::<_,String>(0)?, r.get::<_,i64>(1)?)))?.flatten() {
        top_lines.push(format!("<tr><td>{}</td><td>{}</td></tr>", row.0, format_size(row.1 as u64, BINARY)));
    }
...
<table>{}</table>
</body></html>"#, db_path, top_lines.join(""));
