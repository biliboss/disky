...
pub fn render(frame: &mut Frame, app: &App, list_state: &mut ListState) {
    ...
    let header = Paragraph::new(Line::from(vec![
        Span::styled(" disky ", Style::default().fg(Color::Black).bg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::raw(" "),
    ...
    let items: Vec<ListItem> = app.flat.iter().map(|item| {
    ...
    let help = Paragraph::new(Line::from(vec![
        key("↑↓"), Span::raw(" nav  "),
        key("Enter"), Span::raw(" expand  "),
        key("o"), Span::raw(" Finder  "),
        key("c"), Span::raw(" copy path  "),
        key("e"), Span::raw(" HTML report  "),
        key("r"), Span::raw(" rescan  "),
        key("q"), Span::raw(" quit"),
    ]));
...
fn size_bar(size: i64, max: i64, width: usize) -> String {
    let filled = if max > 0 { (size as f64 / max as f64 * width as f64) as usize } else { 0 };
